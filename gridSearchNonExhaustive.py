...
while (presentGamma >= endGamma):
	...
	#########################################################################
	# TODO:
	# Call gridSearchAuxNonExh.sh from here with the two command line parameters
	# Also, call final run.sh with appropriate command line parameters after appropriate cleanup
	# And ensure that the ranges and values of C and g are matching with paper
	# Test manually
	###########################################################################
	...
	if error == 24:
		...
		print "There was some error, other than error 24."
		########################
...
process = subprocess.Popen(['./removeFiles.sh'])
if process.wait() != 0:
			print "There were some errors"
...
while (presentC >= endC):
	...
	process = subprocess.Popen(['./gridSearchAux0.sh', str(presentC), str(bestGamma_vsOne), str(bestGamma_vsAll), str(bestGamma_dense), str(bestGamma_sparse)])
	error = process.wait();
	if error == 24:
		print "It returned error code ", error
		# Temporarily
	...
	CGammaList_vsOne.append((presentC,bestGamma_vsOne))
	CGammaList_vsAll.append((presentC,bestGamma_vsAll))
	CGammaList_dense.append((presentC,bestGamma_dense))
	CGammaList_sparse.append((presentC,bestGamma_sparse))
	print "#########################################################"
	print "COUNTER IS : ", counter
	print "#########################################################"
	presentC = presentC/intervalC
...
fpVsOne.seek(0)
num_hyper = num_lines/len(CGammaList_vsOne)
hypercounter = 0
...
for row in csvCrossEff:
	...
	if hypercounter%num_hyper == 0:
		if math.isnan(sumCrossEff) is False:
			if (sumCrossEff > maxCrossEff_vsOne):
				(bestC_vsOne,bestGamma_vsOne) = CGammaList_vsOne[(hypercounter/num_hyper) - 1]
				maxCrossEff_vsOne = sumCrossEff
		# For DEBUGGING:
		fpDebugVsOne.write(str(sumCrossEff))
		fpDebugVsOne.write('\n')
...
fpVsAll.seek(0)
num_hyper = num_lines/len(CGammaList_vsAll)
hypercounter = 0
...
for row in csvCrossEff:
	...
	if hypercounter%num_hyper == 0:
		if math.isnan(sumCrossEff) is False:
			if (sumCrossEff > maxCrossEff_vsAll):
				(bestC_vsAll,bestGamma_vsAll) = CGammaList_vsAll[(hypercounter/num_hyper) - 1]
				maxCrossEff_vsAll = sumCrossEff
		# FOR DEBUGGING:
		fpDebugVsAll.write(str(sumCrossEff))
		fpDebugVsAll.write('\n')
...
fpDense.seek(0)
num_hyper = num_lines/len(CGammaList_dense)
hypercounter = 0
...
for row in csvCrossEff:
	...
	if hypercounter%num_hyper == 0:
		if math.isnan(sumCrossEff) is False:
			if (sumCrossEff > maxCrossEff_dense):
				(bestC_dense,bestGamma_dense) = CGammaList_dense[(hypercounter/num_hyper) - 1]
				maxCrossEff_dense = sumCrossEff
		# FOR DEBUGGING:
		fpDebugDense.write(str(sumCrossEff))
		fpDebugDense.write('\n')
...
fpSparse.seek(0)
num_hyper = num_lines/len(CGammaList_sparse)
hypercounter = 0
...
for row in csvCrossEff:
	...
	if hypercounter%num_hyper == 0:
		if math.isnan(sumCrossEff) is False:
			if (sumCrossEff > maxCrossEff_sparse):
				(bestC_sparse,bestGamma_sparse) = CGammaList_sparse[(hypercounter/num_hyper) - 1]
				maxCrossEff_sparse = sumCrossEff
		# FOR DEBUGGING:
		fpDebugSparse.write(str(sumCrossEff))
		fpDebugSparse.write('\n')
...
# processRun = subprocess.Popen(['./runWithTest.sh', str(bestC_vsOne), str(bestGamma_v*sOne), str(bestC_vsAll), str(bestGamma_vsAll), str(bestC_dense), str(bestGamma_dense), str(bestC_sparse), str(bestGamma_sparse)])
...
print ""
print "it took", time.time() - start, "seconds to execute entire script including test"
print ""
